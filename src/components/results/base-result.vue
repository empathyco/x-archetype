  ...
  >
    ...
    <slot name="product-info">
      <BaseResultLink
        ...
        :result="result"
      >
        <h2 class="xds:truncate xds:text-sm xds:uppercase" data-test="result-title">
          {{ result.description }}
        </h2>
        ...
        <div class="xds:flex xds:flex-wrap xds:gap-8">
          <BaseResultCurrentPrice :result="result" class="xds:text-lg xds:text-md xds:font-bold" />
    ...
    <slot name="add-to-cart">
      <BaseAddToCart
        :result="result"
        class="xds:mt-auto xds:button xds:max-h-40 xds:flex-auto xds:rounded-sm xds:border-none xds:bg-neutral-50 xds:text-lg xds:text-md xds:text-neutral-0 xds:hover:bg-neutral-25"
      >
        {{ $t('result.addToCart') }}
      </BaseAddToCart>
    </slot>
